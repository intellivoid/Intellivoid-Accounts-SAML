...
class HTTP
{
     ...
     *
     * @throws Exception If the current session is transient.
    ...
    private static function getSecurePOSTRedirectURL($destination, $data)
    {
        ...
        if ($session->isTransient()) {
            ...
            throw new Exception('Cannot save POST data to a transient session.');
        }
     ...
     * @throws Exception If $url is not a valid HTTP URL.
     *
     * @author Olav Morken, UNINETT AS <olav.morken@uninett.no>
     ...
     *
     * @param Session $session The session where to temporarily store the data.
     ...
     * @throws Exception If the URL is not allowed by configuration.
     *
     * @author Jaime Perez, UNINETT AS <jaime.perez@uninett.no>
     */
     ...
     * @throws Exception If the file or URL cannot be retrieved.
     *
     * @author Andjelko Horvat
     * @author Olav Morken, UNINETT AS <olav.morken@uninett.no>
    ...
    public static function fetch($url, $context = [], $getHeaders = false)
    {
        ...
        if ($data === false) {
            ...
            throw new Exception('Error fetching ' . var_export($url, true) . ':' .
     ...
     * @throws CriticalConfigurationError If 'baseurlpath' has an invalid format.
     *
     * @author Olav Morken, UNINETT AS <olav.morken@uninett.no>
     */
     ...
     * @return void
     *
     * @throws CannotSetCookie If the headers were already sent and the cookie cannot be set.
     *
     ...
     * @return void
     *
     * @throws Exception If $destination is not a valid HTTP URL.
     *
     * @throws InvalidArgumentException If $destination is not a string or $data is not an array.
     * @author Olav Morken, UNINETT AS <olav.morken@uninett.no>
     * @author Andjelko Horvat
